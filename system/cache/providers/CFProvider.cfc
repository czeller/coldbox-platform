...
*/
component serializable="false" implements="cachebox.system.cache.ICacheProvider"{
    ...
    */
	CFProvider function init() output=false{
		...
		instance = {
			...
			elementCleaner		= CreateObject("component","cachebox.system.cache.util.ElementCleaner").init(this),
			// Utilities
			utility				= createObject("component","cachebox.system.core.util.Util"),
	...
	* @doc_generic cachebox.system.cache.util.ICacheStats
	*/
	any function getStats() output=false{
		return CreateObject("component", "cachebox.system.cache.providers.cf-lib.CFStats").init( getObjectStore().getStatistics() );
	}
    ...
    */
    any function setQuiet(required any objectKey,
		...
		// use configured defaults for timeout/lastAccessTimeout
		if( NOT len(arguments.timeout) OR NOT isNumeric(arguments.timeout) ){
			arguments.timeout = cacheConfig.timeToIdleSeconds;
		}

		if( NOT len(arguments.lastAccessTimeout) OR NOT isNumeric(arguments.lastAccessTimeout) ){
			arguments.lastAccessTimeout = cacheConfig.timeToLiveSeconds;
		}

		// check if incoming timeout is a timespan or second to convert to timespan
		if( findnocase("string", arguments.timeout.getClass().getName() ) ){
			arguments.timeout = createTimeSpan(0,0,0,arguments.timeout);
		}

		if( findnocase("string", arguments.lastAccessTimeout.getClass().getName() ) ){
			arguments.lastAccessTimeout = createTimeSpan(0,0,0,arguments.lastAccessTimeout);
		}
